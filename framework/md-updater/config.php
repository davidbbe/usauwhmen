...
define( 'NOTIFIER_THEME_NAME', MD_THEME_NAME ); // The theme name
define( 'NOTIFIER_THEME_FOLDER_NAME', strtolower(MD_THEME_NAME) ); // The theme folder name
define( 'NOTIFIER_XML_FILE', 'http://demo.themesholic.com/md-updater.php?t='.NOTIFIER_THEME_NAME ); // The remote notifier XML file containing the latest version of the theme and changelog
define( 'NOTIFIER_CACHE_INTERVAL', 1 ); // The time interval for the remote XML cache in the database (21600 seconds = 6 hours)
define( 'NOTIFIER_VENDOR', 'WrapBootstrap');
define( 'NOTIFIER_VENDOR_URL', 'http://www.wrapbootstrap.com');
...
function update_notifier_menu() {  
	if (function_exists('simplexml_load_string')) { // Stop if simplexml_load_string funtion isn't available
		...
		$theme_data = wp_get_theme(MD_THEME_NAME); // Read theme current version from the style.css
		
		if( (float)$xml->latest > (float)$theme_data['Version']) { // Compare current theme version with the remote XML version
			add_dashboard_page( strtoupper(NOTIFIER_THEME_NAME) . ' Theme Updates', strtoupper(NOTIFIER_THEME_NAME) . ' <span class="update-plugins count-1"><span class="update-count">New Updates</span></span>', 'administrator', 'theme-update-notifier', 'update_notifier');
		}
	}	
}
...
function update_notifier_bar_menu() {
	if (function_exists('simplexml_load_string')) { // Stop if simplexml_load_string funtion isn't available
		...
		if( (float)$xml->latest > (float)$theme_data['Version']) { // Compare current theme version with the remote XML version
			$wp_admin_bar->add_menu( array( 'id' => 'update_notifier', 'title' => '<span>' . strtoupper(NOTIFIER_THEME_NAME) . ' <span id="ab-updates">New Update</span></span>', 'href' => get_admin_url() . 'index.php?page=theme-update-notifier' ) );
		}
	}
}
...
function update_notifier() { 
	...
	<div class="wrap">
		...
		<h2><?php echo strtoupper(NOTIFIER_THEME_NAME) ?> Theme Updates</h2>
		...
		<div id="instructions">
		    ...
		    <p>To update the Theme, login to <a href="<?php echo NOTIFIER_VENDOR_URL; ?>"><?php echo NOTIFIER_VENDOR; ?></a>, head over to your <strong>downloads</strong> section and re-download the theme like you did when you bought it.</p>
